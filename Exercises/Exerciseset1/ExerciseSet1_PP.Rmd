...
#### Answer 1:  
It is done in the [dataSet1.R file](https://github.com/InseadDataAnalytics/INSEADAnalytics/tree/master/Exercises/Exerciseset1/dataSet1.R) in line 8.
...
#### Answer 3:  
1. The cumulative returns of Yahoo during this period is `r round(100*sum(StockReturns[,3]),1)`%.  
2. The average daily returns of Yahoo during this period is `r round(100*mean(StockReturns[,3]),3)`%;  
3. The standard deviation of the daily returns of Yahoo during this period is `r round(100*sd(StockReturns[,3]),3)`%;  
<br>
<br>
<br>
...
portfolio = apply(StockReturns[,1:2],1,mean)
names(portfolio) <- rownames(StockReturns[,1:2])
pnl_plot(portfolio)
```
...
*Rows:*  `nrow(transposedData)` or `ncol(StockReturns)`

*Collumns:*  `ncol(transposedData)` or `nrow(StockReturns)`
...
1. We have to change `startDate = "2001-01-01"` in the [dataSet1.R](https://github.com/InseadDataAnalytics/INSEADAnalytics/tree/master/Exercises/Exerciseset1/dataSet1.R) file line 9.  

2. Portfolio of Volkswagen AG, Gilead Sciences Inc, and Tesla Motors Inc from 06/08/2015 until today:  
```{r echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig=TRUE}
portfolio = apply(StockReturns[2666:nrow(StockReturns),4:6],1,mean)
names(portfolio) <- rownames(StockReturns[2666:nrow(StockReturns),4:6])
pnl_plot(portfolio)
```
<br>
<br>
...
The system compares the variable "myData" with the first 20 rows of the matrix "StockReturns". If both data-sets are the same the sum of results should be 0. Here the result is `r sum(myData != StockReturns[1:20,])` because the first collumn of "myData" consists of the dates which are not part of the data in "StockReturns".


This is also the reason why it is also not possible to add the two matrixes. The "dates collumn" of "myData" cannot be added to the numerical "SPY collumn" of "StockReturns".
<br>
<br>
<br>
<br>

...
1. We need at least 6 principal components.
2. The highest weights are 0.93 of 5 yr T-Notes US and 10yr T-Notes.
3. Yes we can interpret them:
  1: Bonds (US focused), 2: Currencies, 3: Indexes (EU focused), 4: Indexes (US focused), 5: Bonds (DEU focused), 6: Oil/Gas, 7: Base metals, 8: Grains, 9: Noble metals, 10: Indexes (Asia)  
4.  
***Pre-crisis years:***
...
Variance_Explained_Table3_results<-PCA(pre_crisis_data, graph=FALSE)
Variance_Explained_Table3<-cbind(paste("component",1:ncol(pre_crisis_data),sep=" "),Variance_Explained_Table3_results$eig)
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data3 = data.frame(Variance_Explained_Table3)
m1<-gvisTable(show_data3,options=list(width=1920, height=min(400,27*(nrow(show_data3)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
Scree plot:
```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
...
```{r Fig13, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
df3           <- cbind(as.data.frame(eigenvalues3), c(1:length(eigenvalues3)), rep(1, length(eigenvalues3)))
colnames(df3) <- c("eigenvalues", "components", "abline")
Line3         <- gvisLineChart(as.data.frame(df3), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line3, 'chart')
```

```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis',tidy=TRUE}
corused3 = cor(pre_crisis_data[,apply(pre_crisis_data!=0,2,sum) > 10, drop=F])
...
colnames(Rotated_Factors3)<-paste("Component",1:ncol(Rotated_Factors3),sep=" ")

sorted_rows3 <- sort(Rotated_Factors3[,1], decreasing = TRUE, index.return = TRUE)$ix
Rotated_Factors3 <- Rotated_Factors3[sorted_rows,]
Rotated_Factors3[abs(Rotated_Factors3) < 0.3]<-NA
```

```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
show_data3 <- Rotated_Factors3 
show_data3<-cbind(rownames(show_data3),show_data3)
colnames(show_data3)<-c("Variables",colnames(Rotated_Factors3))
m1<-gvisTable(show_data3,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data3)+1)),allowHTML=TRUE,page='disable'))
print(m1,'chart')
```
...
Variance_Explained_Table2_results<-PCA(crisis_data, graph=FALSE)
Variance_Explained_Table2<-cbind(paste("component",1:ncol(crisis_data),sep=" "),Variance_Explained_Table2_results$eig)
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE, results='asis'}
show_data2 = data.frame(Variance_Explained_Table2)
m1<-gvisTable(show_data2,options=list(width=1920, height=min(400,27*(nrow(show_data2)+1)),allowHTML=TRUE,page='disable'),formats=list(Eigenvalue="#.##",Percentage_of_explained_variance="#.##",Cumulative_percentage_of_explained_variance="#.##"))
print(m1,'chart')
```
Scree plot:
```{r echo=FALSE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
...
```{r Fig12, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
df2           <- cbind(as.data.frame(eigenvalues2), c(1:length(eigenvalues2)), rep(1, length(eigenvalues2)))
colnames(df2) <- c("eigenvalues", "components", "abline")
Line2         <- gvisLineChart(as.data.frame(df2), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line2, 'chart')
```
...
```{r echo=FALSE, comment=NA, warning=FALSE, error=FALSE,message=FALSE,results='asis'}
show_data2 <- Rotated_Factors2 
show_data2<-cbind(rownames(show_data2),show_data2)
colnames(show_data2)<-c("Variables",colnames(Rotated_Factors2))
m1<-gvisTable(show_data2,options=list(showRowNumber=TRUE,width=1220, height=min(400,27*(nrow(show_data2)+1)),allowHTML=TRUE,page='disable'))
print(m1,'chart')
```

5. During the crisis years futures returns in general were more correlated. 4 principal components were enough to explain 50% of the variance. Especially among commodities there seems to be a higher correlation during bear years.
...
1. The code creates a couple of moving averages (`r seq(10,250,by=20)`) and adds their signs (+ or -) into the empty variable "signal_used" to indicate whether it should buy on that day. Then it scales the results to a sum of 1 so that the investment is the same every day and does not change with the number of positive results. Finally the results are transformed into the trading strategy which uses the signal from 2 days ago.

2. The following trading strategy uses smaller moving averages (`r seq(10,120,by=10)`). That means it reacts more on short term movements. The results are almost the same.
```{r echo=TRUE, eval=TRUE, comment=NA, warning=FALSE,error=FALSE, message=FALSE, prompt=FALSE, tidy=TRUE}
signal_used2 = 0*futures_data # just initialize the trading signal to be 0
# Take many moving Average (MA) Signals and let them "vote" with their sign (+-1, e.g. long or short vote, for each signal)
MAfreq2<-seq(10,120,by=10)
for (iter in 1:length(MAfreq2))
  signal_used2 = signal_used2 + sign(apply(futures_data,2, function(r) ma(r,MAfreq2[iter])))
# Now make sure we invest $1 every day (so the sum of the absolute values of the weights is 1 every day)
signal_used2 = t(apply(signal_used2,1,function(r) {
  res = r  
  if ( sum(abs(r)) !=0 )
    res = r/sum(abs(r))
  res
...
# Now create the returns of the strategy for each futures time series
strategy_by_future2 <- scrub(shift(signal_used2,2)*futures_data) # use the signal from 2 days ago
# finally, this is our futures trend following strategy
